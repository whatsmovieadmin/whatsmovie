...
public class Principal extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		Typeface pacifico = Typeface.createFromAsset(getAssets(),
				"fonts/Pacifico.ttf");
		Typeface walkway_semibold = Typeface.createFromAsset(getAssets(),
				"fonts/Walkway_SemiBold.ttf");

		EditText et_email = (EditText) findViewById(R.id.input_buscar_usuario);
		...
		if (!isRemember) {
			...
			if (!networkAvailable()) {
				AlertDialog alerta = new AlertDialog.Builder(Principal.this)
						.create();
		...
		Button bt_registrar = (Button) findViewById(R.id.bt_amigos_aniadir);
		...
		bt_entrar.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View view) {
				...
				if (!networkAvailable()) {
					...
					try {
						email = (EditText) findViewById(R.id.input_buscar_usuario);
						...
						if (!a.contains("error")) {
							...
							SharedPreferences myPrefs = Principal.this
									.getSharedPreferences("myPrefs",
											MODE_WORLD_READABLE);
							SharedPreferences.Editor prefsEditor = myPrefs
									.edit();
							prefsEditor.putString("LOGIN_EMAIL", email
									.getText().toString());
							prefsEditor.putString("LOGIN_PSWD", pwd.getText()
									.toString());
							...
							String resultSize = a.substring(
									a.indexOf("<error>") + 7,
									a.indexOf("</error>"));

							Toast t = Toast.makeText(getApplicationContext(),
									resultSize, Toast.LENGTH_SHORT);
							t.show();
						}
	...
	class PrincipalTask extends AsyncTask<EditText, Void, String> {
		...
		protected String doInBackground(EditText... params) {
			...
			try {
				...
				URL url = new URL(
						"http://movies-quiz.appspot.com/webservices/Login?&pwd="
								+ params[1].getText().toString() + "&email="
								+ params[0].getText().toString());
